...
describe("PBXNativeTarget", () => {
  ...
  describe("create", () => {
    it("should create new native target with minimal options", () => {
      ...
      const existingTarget = project.props.targets.find(t => PBXNativeTarget.is(t)) as PBXNativeTarget;
      ...
      expect(newTarget.props.productType).toBe("com.apple.product-type.framework");
  ...
  describe("isReferencing", () => {
    it("should return true for build rule UUIDs", () => {
      const xcproj = XcodeProject.open(WORKING_FIXTURE);
      const target = xcproj.getObject("299522761BBF136400859F49") as PBXNativeTarget;
    ...
    it("should return true for product reference UUID", () => {
      ...
      if (target.props.productReference) {
        expect(target.isReferencing(target.props.productReference.uuid)).toBe(true);
      }
    });
  ...
  describe("ensureFrameworks", () => {
    ...
    it("should handle frameworks without .framework extension", () => {
      ...
      const uikitRef = frameworksFolder.props.children.find(child => 
        PBXFileReference.is(child) && child.props.name === "UIKit.framework"
      );
      const foundationRef = frameworksFolder.props.children.find(child => 
        PBXFileReference.is(child) && child.props.name === "Foundation.framework"
      );
    ...
    it("should add frameworks to Frameworks group", () => {
      ...
      expect(frameworksFolder.props.children.length).toBeGreaterThan(initialChildCount);
      
      const cloudKitRef = frameworksFolder.props.children.find(child => 
        PBXFileReference.is(child) && child.props.name === "CloudKit.framework"
      );
  ...
  describe("addDependency", () => {
    it("should add dependency between targets in same project", () => {
      ...
      expect(mainTarget!.props.dependencies.length).toBe(initialDependencyCount + 1);
      
      // Check that the dependency points to the correct target
      const newDependency = mainTarget!.props.dependencies[mainTarget!.props.dependencies.length - 1];
    ...
    it("should not add duplicate dependencies", () => {
      ...
      const extensionTarget = xcproj.rootObject.getNativeTarget("com.apple.product-type.app-extension");
    ...
    it("should create container item proxy for dependency", () => {
      ...
      const dependency = mainTarget!.props.dependencies[mainTarget!.props.dependencies.length - 1];
      expect(dependency.props.targetProxy).toBeDefined();
      expect(dependency.props.targetProxy.props.remoteGlobalIDString).toBe(newTarget.uuid);
      expect(dependency.props.targetProxy.props.remoteInfo).toBe("TestDependency");
  ...
  describe("getCopyBuildPhaseForTarget", () => {
    it("should create copy build phase for app extension", () => {
      ...
      const copyPhase = mainTarget!.getCopyBuildPhaseForTarget(extensionTarget!);
    ...
    it("should create copy build phase for ExtensionKit extensions", () => {
      ...
      const copyPhase = mainTarget!.getCopyBuildPhaseForTarget(extensionKitTarget);
    ...
    it("should reuse existing copy build phase", () => {
      ...
      const copyPhase1 = mainTarget!.getCopyBuildPhaseForTarget(extensionTarget!);
      const copyPhase2 = mainTarget!.getCopyBuildPhaseForTarget(extensionTarget!);
    ...
    it("should throw error when called on non-main target", () => {
      ...
      }).toThrow("getCopyBuildPhaseForTarget can only be called on the main target");
    });
  });
  ...
  describe("integration tests", () => {
    it("should handle complex target relationships", () => {
      ...
      expect(mainTarget!.getFrameworksBuildPhase().props.files.length).toBeGreaterThan(0);
      expect(extensionTarget!.getFrameworksBuildPhase().props.files.length).toBeGreaterThan(0);
